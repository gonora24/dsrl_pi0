...
class Agent(object):
    ...
    def sample_actions(self, observations: np.ndarray, marginalize_logprobs: bool = False, use_actor_diff: bool = False, use_actor_diff_mean: bool = False) -> np.ndarray:
        rng, actions = sample_actions_jit(self._rng, self._actor.apply_fn,
                                          self._actor.params, observations, get_batch_stats(self._actor), marginalize_logprobs, use_actor_diff, use_actor_diff_mean)
